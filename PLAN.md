...
| Воспроизводимость сдачи | Каталоги `data/` и `outputs/` сейчас игнорируются Git, включая официальный `data/README.md`. При реализации B уточняет gitignore так, чтобы документация датасета учитывалась отдельно от parquet, и проверяет правила передачи dataset/CSV; P описывает получение данных. Не считать игнорируемые локальные файлы отправленными жюри |
