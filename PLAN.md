# Money Graph — план на оставшиеся 3 часа 20 минут

> Исторический план на момент перепланирования. Текущий статус (2026-09-23):
> `pipeline.py` реализован, официальный набор из `../Финансы/data/data`
> прошёл независимый validator, два запуска дали одинаковые CSV и метрики,
> полный расчёт занял 2,38 и 2,27 с. Аудит результатов и ограничений —
> в [AUDIT_LANE_A.md](AUDIT_LANE_A.md), команды — в [README.md](README.md).
> Утверждения ниже о будущем pipeline и отсутствии реальной интеграции
> относятся к исходной точке этого плана.

Статус: **реализация идёт; пользователь — Technical Lane B**. Осталось **200 минут на момент сообщения пользователя об оставшемся времени**. Здесь T+0 — это сообщение, не начало старого пятичасового плана; время на перепланирование входит в бюджет. Прежние H1–H5 больше не действуют. Команда: два технических участника и один Product / QA / Demo.

**Критический путь:** готовая выгрузка A → реальный UI B → проверка и воспроизводимость → сдача/демо. Lane B уже реализована и проверена на synthetic fixture (`eddb72b`); это не подтверждает готовность полного решения. **Все optional сняты с текущей сдачи.** Первое подключение реального результата — T+40; фиксация функциональности — T+140; последние 30 минут защищены для резерва и сдачи.
...
- **Lane B выполнена отдельно от pipeline:** `app.py`, `src/view.py`, `validate.py`, `run-ui.sh`, конфигурация Streamlit, `requirements.lock`; 46 pytest/AppTest проверок прошли также в чистом окружении, Chromium smoke прошёл. SciPy установлен. UI показывает явно помеченный синтетический пример. Формат передачи и команды — `TECHNICAL_LANE_B.md`.
- **Не подтверждено в текущем checkout:** запуск A от parquet до готовой выгрузки, реальные роли/кластеры для UI, итоговый runtime и повторяемость pipeline. `pipeline.py` пока нет; `src/data.py` и `src/engine.py` — черновики. Это не утверждение о прогрессе A в другом checkout: статус нужно сверить на T+10.
- Корневой README, схема решения, demo-script и QA-протокол подготовлены в `README.md` и `docs/`. Они не подменяют реальный прогон: фактические runtime, `run_id`, Top-узлы и приёмка произвольных gid заполняются только после поставки опубликованного bundle A.
...
| MUST: граф, направление, роли/кластеры, поиск и связи | Произвольный gid, фильтры, вход/выход | `app.py`, QA-протокол | B | Реализовано; проверено на synthetic, ожидается реальная интеграция |
...
| MUST: локально, без hardcoded gid/атрибутов/облака | Чистый запуск, ревью зависимостей | Исходники, закреплённые зависимости | B | Окружение B проверено; полный pipeline A ещё нет |
| MUST: исходники, README, диаграмма, 5-минутное демо | Проход по инструкции другим участником | README, `docs/architecture.md`, `docs/demo.md` | P | Документы готовы; фактическая репетиция ждёт bundle A |
| SCORE: независимые тесты и повторяемость | Граничные случаи и повторный прогон | `tests/`, `validate.py`, QA-отчёт | B | 46 тестов прошли; повторяемость A ещё не проверена |
| SCORE: понятность, роль ≠ приоритет, раскрытие вкладов | Пользователь объясняет выбранный узел | Карточка с метриками и score breakdown | B | UI готов; нужны реальные метрики и QA объяснений |
| SCORE: методология, масштаб ~1M, осторожный AML-язык | Проверка соответствия работающему коду | README, вопросы жюри | P | Описано; соответствие финальному pipeline ждёт bundle A |
| OPTIONAL: временное подтверждение транзита | Проверка дневного FIFO-сопоставления | Дополнительный temporal-признак | A | Отложено за пределы текущей сдачи |
| OPTIONAL: устойчивость Top к весам приоритета | Сравнение Top 20 при небольших изменениях весов | Короткий диагностический отчёт | A | Отложено за пределы текущей сдачи |
...
К **T+40 от сообщения об остатке времени**: реальные parquet → граф со всеми узлами → метрики/Louvain → первая законченная версия правил/приоритета → три CSV → подключение **уже готового** UI B → поиск реального gid, evidence и направленные связи.

Не разрабатывать UI заново и не ждать идеальной методологии для первого подключения. Адаптацию формата в зоне reader B выполнить сразу после поставки A. К T+90 устранить ошибки правил/ограничений и получить зелёную обязательную валидацию. Mock не считается интеграцией; отсутствие изолятов или placeholder CSV блокирует сдачу.
...
**Статус передачи.** Контракт A получен и объединён с Lane B. Независимый validator и UI B проверены на fixture v1.0.0; чтение current.json, candidate без run.json и защитные проверки покрыты тестами. Публикация реальным pipeline A и реальные CSV ещё не подтверждены. Существующие data.py/engine.py не соответствуют v1: строковые ID, старые имена метрик, старые scoring/temporal, отсутствие component_id и полей неоднозначности. A адаптирует их следующим этапом; сам факт наличия contracts.py не означает соответствия outputs контракту.
...
- **Зависимости:** B предоставляет validator к T+40 и установленные зависимости; P — acceptance-кейсы к T+10. Пока ждёт, A проверяет ручными пересчётами и собственными маленькими графами, готовит реальные outputs по согласованной схеме.
- **Сразу:** аудит черновика против starter/реальных данных; спецификация полей/порогов; полный набор nodes в графе.
- **Ближайшая интеграция T+40:** первая полностью заполненная выгрузка всех 2248, метрики/edges/run manifest для B, список известных методологических недоработок.
...
- **Владелец:** пользователь (технический участник B); отвечает за UI, независимую проверку, упаковку и ревью кода своего агента.
...
- **Зависимости:** A передаёт contract v1 к T+10, реальные bundle к T+35; P — flow и acceptance. Reader, validator, UI и synthetic fixture уже готовы. Пока ждёт, B сопоставляет названия полей и передаёт A минимальный manifest; сохраняет проверки synthetic как regression, не выдавая их за реальную интеграцию.
- **Сразу:** согласовать с A reader/manifest, убрать несовпадения имён полей в адаптере B; новых экранов и оформления не делать. При поставке A сразу запускать validator и открывать произвольный реальный gid.
- **Ближайшая интеграция T+40:** читает реальные файлы A, показывает Top, поиск, метрики и направленные связи одного любого gid; validator проверяет базовую полноту.
...
- **Ближайшая интеграция T+40:** `docs/acceptance.md` и первый QA-протокол; README с фактически проверенными командами; проверить counts/CSV A и lookup/card/edges B. Результат — pass/fail и баги со входом, шагами, expected/actual, severity.
...
Уже выполненный вклад B: `eddb72b` — UI, validator, тесты и инструкция передачи. Следующие поставки: A — реальные CSV/bundle к T+35 и исправленные расчёты к T+90; B — реальная интеграция T+40 и полный clean-run T+140; P — acceptance/README draft T+40, QA-log на каждом checkpoint, готовые diagram/demo к T+140. Сохранять реальное авторство, не создавать фиктивные коммиты. Пользователь попросил коммитить готовые изменения сразу; push отдельно не разрешён.
...
## 10. Оставшиеся 200 минут — Integration checkpoints

P0 = все MUST, включая UI и документацию. **P1/P2 сняты.** Уже готовую B не переделываем. Все три участника работают параллельно; ожидание результата другой линии заменяется полезной работой из таблицы.

| От текущего сообщения / остаток на выходе | A: поставка | B (пользователь): поставка | P: проверка обеих линий / evidence | При срыве |
|---|---|---|---|---|
| 0–10 мин / 3:10 | Сообщить фактический прогресс, подтвердить поля и команду запуска; выделить недостающие части расчёта | Передать `TECHNICAL_LANE_B.md`, сверить reader и manifest с A; список несовпадений | MUST-checklist, структура README и 5-минутного демо; проверка полей A и команды B | Зафиксировать минимальный формат; никаких новых модулей ради архитектуры |
| 10–40 мин / 2:40 | До T+35: полный первый расчёт 2248 узлов, 3 CSV + metrics/edges/manifest | До T+40: подключить реальные файлы, запустить validator, показать поиск/направления/карточку | Counts/evidence A; поиск случайного gid, boundary и изолята B; QA-log pass/fail | 20 минут совместно на конкретный blocker; B чинит reader/упаковку, A — экспорт/расчёт; mock не считается успехом |
| 40–90 мин / 1:50 | Исправить role/priority/evidence, boundary/seed, изоляты, кластеры; повторный запуск | Закрыть интеграционные ошибки; независимая сверка CSV/метрик, настоящие фильтры/Top/поиск | 3 произвольных gid за минуту, распределения ролей и ограничения; README формул соответствует коду | Сохранить простые объяснимые правила; не менять стек и не ослаблять проверки. Если полного прохода нет к T+60 — обе технические линии занимаются только его восстановлением |
| 90–140 мин / 1:00 | Финальная базовая методология; два запуска, одинаковые CSV, замер <300с | Полный clean-run по инструкции и browser smoke на настоящих данных; подготовить файлы сдачи | Проверка запуска A/UI B другим участником; готовые README, схема, сценарий и Q&A | Исправлять только обязательные ошибки; никакой косметики и новых признаков |
| 140–170 мин / 0:30 | **Заморозка функций T+140**; исправления blocker и финальная выгрузка | Проверить финальный manifest/CSV/UI и локальный комплект сдачи | Полная 5-минутная репетиция + произвольные gid; сверка обязательных артефактов | Использовать последнюю проверенную реальную выгрузку для демо, явно называя её сохранённой; воспроизводимый pipeline остаётся обязательным |
| 170–200 мин / дедлайн | Только исправление критического дефекта с повтором затронутых проверок | Защищённый резерв: окончательная проверка комплекта, путей и доступа на машине демо | Сдача по правилам организаторов, проверка доступности файлов, краткая репетиция | Никаких новых функций, зависимостей или рефакторинга; не выдавать непроверенные результаты за проверенные |

Последние 30 минут не расходуем на optional. Время экономим за счёт сокращения объёма и использования готового B, не отказа от runtime-проверки или README (25 баллов).

## 11. Ближайшая реальная интеграция

**T+40 от сообщения об остатке времени.** A отдаёт результаты из реальных `data/`: три CSV, `node_metrics.parquet`, `edges.parquet`, `run.json`. B подключает их к уже работающим validator/UI; несовпадение технических имён решается одним адаптером после сверки с A, без двух независимых вариантов расчёта.

P проверяет обе поставки: 2248 строк и точное покрытие gid, заполненные поля, ≥20 Top, кластеры и фактические числа; затем случайный gid, boundary и изолят в UI. Evidence должно совпадать с рассчитанными метриками, стрелки — с направлением edges. Фиксируются команда, runtime, pass/fail и конкретные баги с владельцем. Это первый проход; методологический аудит заканчивается к T+90.

Если checkpoint сорван: ближайшие 20 минут A/B исправляют единственный критический путь, P повторяет те же кейсы и продолжает README. B не берёт незаметно код engine A; помощь в экспортёре/запуске — после явной передачи конкретного файла, без параллельного редактирования. Внешний прогресс A не предполагается: нужен доступный результат, а не статус «почти готово».

## 12. Optional feature decision

**Для оставшихся 3:20 отключены оба ранее выбранных расширения:** temporal-подтверждение transit и исследование чувствительности Top. LLM, дополнительные графовые паттерны и новый UI также не планируются. Уже существующий temporal-код не обязан входить в сдачу и не должен становиться зависимостью P0; A решает сохранить только проверенные части без изменения критического пути.

Освободившееся время направляется на объяснение произвольных gid, фактическую интеграцию, два воспроизводимых запуска, README и демо. Идеи расширений можно кратко упомянуть в разделе перспектив, явно как нереализованные.
...
|---|---|
| Слабые пороги и пересечение ролей | Квантили из реальных данных, смысловые gates, secondary score/margin, аудит к T+90, no quotas; A + проверка P |
...
| Расхождение контрактов/двух стартеров | A — единственный владелец schema, reuse starter, одна точка запуска, реальная интеграция T+40; A/B |
| Нечитаемый граф | Обзор кластеров/компонент, ограниченный ego, стрелки, полные таблицы и явное усечение отображения; B |
| Runtime и зависимости | Приближённый betweenness, замеры стадий, SciPy/версии, чистый запуск к T+140; A/B |
...
Предположения: хватает локального Python-окружения и времени для установки зависимостей; benchmark делается на текущем наборе; текущие веса — документируемая эвристика. Остаток 200 минут привязан к сообщению пользователя, а не возобновляется после чтения этого файла. Точные требования к каналу сдачи/публикации данных не предоставлены: готовим локальные артефакты, готовые изменения коммитим по указанию пользователя; автоматический push не разрешён. Реализация Lane B уже разрешена; код A остаётся ответственностью его владельца.
