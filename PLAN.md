...
- **Зависимости и ожидание:** код/bundle A уже получены; P уточняет машину показа и правила сдачи. Пока установки/ответы ожидаются, сверить состав пакета, пути и Git-tracked файлы; подготовить инструкцию P. Не считать локальные игнорируемые outputs отправленными.
...
- **Сразу / поставка к T+25:** пройти текущий реальный UI по `docs/qa.md`: Top, три произвольных gid, активные фильтры, 1–2 шага, boundary, seed/изолят, скачивания. Заполнить таблицу pass/fail фактами; непроверенные пункты оставить открытыми. Уточнить точный канал/формат сдачи и что жюри получает с данными.
...
| Игнорируемые data/outputs отсутствуют у жюри | P выясняет правила передачи; B готовит три CSV и полный bundle одного run отдельно. Не считать локальные файлы частью push |
...
Предположения: 120 минут включают последние 30 минут проверок/сдачи; если организаторы требуют более раннюю загрузку, P переносит отправку раньше. Официальный канал и правила публикации данных нужно подтвердить у организаторов; публичную публикацию датасета не предполагаем. Базовый pipeline проверен локально на Python 3.12; чистый полный запуск текущей версии и машина показа ещё требуют отдельного подтверждения. Изменение PLAN.md разрешено пользователем; код аналитики остаётся ответственностью A.
