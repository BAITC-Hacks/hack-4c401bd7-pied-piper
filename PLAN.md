...
- **Lane B выполнена отдельно от pipeline:** `app.py`, `src/view.py`, `validate.py`, `run-ui.sh`, конфигурация Streamlit, `requirements.lock`; 23 pytest/AppTest проверки прошли также в чистом окружении, Chromium smoke прошёл. SciPy установлен. UI показывает явно помеченный синтетический пример. Формат передачи и команды — `TECHNICAL_LANE_B.md`.
...
- Корневой README пока краткий; итоговые QA/demo-документы Product в текущем checkout не обнаружены. B передаёт готовую техническую инструкцию P, не переписывает документацию параллельно. На этом перепланировании меняется только PLAN.md.
...
| SCORE: независимые тесты и повторяемость | Граничные случаи и повторный прогон | `tests/`, `validate.py`, QA-отчёт | B | 23 теста прошли; повторяемость A ещё не проверена |
...
**Контракт A v1.0.0 зафиксирован в `src/contracts.py`.** Модуль содержит декларативные схемы, порядок ролей, веса, допуски и структуру manifest; импортируется без pandas/NetworkX. Это спецификация обмена, не реализация pipeline. До T+10 A и B сверяют этот контракт с уже реализованным reader B: прототип принимает `schema_version=1` и плоский каталог, контракт A требует `schema_version="1.0.0"`, `counts`, `current.json`/`runs/<run_id>` и режим `--candidate`. Эти расхождения — открытая задача интеграции; прохождение прежних 23 тестов B не подтверждает поддержку v1.0.0. Сроки оставшихся 200 минут сохраняются. P проверяет соответствие ТЗ. У каждого контракта ниже один технический владелец — **A**; потребители — B (validator/UI) и P (acceptance). Изменения только через явную передачу версии, без параллельного редактирования.
...
**Статус передачи.** Контракт A получен и объединён с Lane B. Независимый validator и UI B уже проверены на прежнем synthetic fixture; чтение fixture по v1.0.0, режим candidate, публикация bundle и реальные CSV ещё не подтверждены. Существующие data.py/engine.py не соответствуют v1: строковые ID, старые имена метрик, старые scoring/temporal, отсутствие component_id и полей неоднозначности. A адаптирует их следующим этапом; сам факт наличия contracts.py не означает соответствия outputs контракту.
