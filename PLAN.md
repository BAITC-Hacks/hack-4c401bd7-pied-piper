...
До T+10 A и B сверяют существующий reader и минимальный manifest из `TECHNICAL_LANE_B.md`; A подтверждает поля своей выгрузки, P проверяет обязательные результаты. Отдельный `src/contracts.py` создавать только если это помогает существующему коду; его отсутствие не должно задерживать передачу файлов. У каждого контракта ниже один технический владелец — **A**; потребители — B (validator/UI) и P (acceptance). Изменения только через явную передачу версии, без параллельного редактирования.
