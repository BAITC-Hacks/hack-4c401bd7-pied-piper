...
5. Аналитик открывает обзор, выбирает кандидата из Top или вводит любой `gid`, изучает карточку и направления переводов, затем выгружает CSV для дальнейшей проверки.
...
| Streamlit 1.49.1, Plotly 6.3.0 | Интерфейс и интерактивные графы |
...
2. На «Обзоре сети» проверьте показатели официального набора: **2 248 клиентов, 3 119 связей, 105 кластеров и 35 компонент**.
3. Откройте «Приоритет проверки», выберите одного из первых клиентов и скопируйте его `gid`.
...
7. Скачайте `top_nodes.csv`, `nodes_roles.csv` и `clusters.csv`. Проверьте, что роли покрывают 2 248 клиентов, а Top содержит 20 строк.
...
После расширения покрытия полный локальный прогон `python -m pytest -q` дал **79 passed за 20,49 с**. Добавлены проверки шести ролей и их объяснений, запрета transit для seed, запрета terminal на границе, порогов по квантилям, конкурирующих ролей, изолятов, порядка Top, сумм кластеров и строгого ограничения runtime <300 секунд. Время зависит от окружения.
...
- [План и подробные правила](PLAN.md)
