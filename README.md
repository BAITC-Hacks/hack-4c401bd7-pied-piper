...
- Backend API и отдельный worker: [запуск, хранение и ограничения](docs/backend.md).
...
- Шесть ролей с числовой поддержкой правилами, альтернативной ролью и объяснением `evidence`.
...
- Симуляция устойчивости сети: исключение top-1/3/5/10 по приоритету и сравнение с отбором по обороту; распад связности и затронутые суммы без изменения результатов.
...
[Контракт API](docs/frontend-contract.md); машинная схема сервера: `/openapi.json`.
...
```mermaid
flowchart LR
    D["data/*.parquet"] --> P["pipeline.py: загрузка и расчёт"]
    P --> S["outputs/.staging: кандидат"]
    S --> V["validate.py: независимая проверка"]
    V --> R["outputs/runs: опубликованный результат"]
    R --> C["current.json: указатель на запуск"]
    C --> U["app.py: обзор, Top, поиск и связи"]
    U --> E["CSV для аналитика"]
```

| Компонент | Ответственность |
| --- | --- |
| `src/data.py` | Загрузка и проверка исходных таблиц |
| `src/engine.py` | Граф, сообщества, признаки, роли и приоритет |
| `src/reporting.py` | Таблицы результатов и текстовые объяснения |
| `src/contracts.py` | Схемы v1.0.0, типы, роли, веса и допуски |
| `pipeline.py` | Последовательность расчёта, проверка и публикация |
| `validate.py` | Проверка результата независимо от engine |
| `src/bundle_io.py`, `src/view.py` | Чтение публикации, проверка целостности и построение графиков |
| `src/results.py` | Чтение и проверка bundle для backend и validator без визуализации |
| `backend/api.py`, `backend/store.py`, `backend/worker.py` | HTTP API, SQLite-реестр и отдельный обработчик заданий |
| `src/graph_ui.py`, `src/graph.html` | Интерактивный SVG-граф, перетаскивание узлов и компактная раскладка |
| `src/presentation.py` | Понятные пояснения к рассчитанным ролям и приоритетам |
| `src/resilience.py` | Read-only сравнение устойчивости полной сети при исключении ключевых узлов |
| `app.py` | Рабочее место аналитика |
...
Исходные Parquet и сохранённый результат включены в репозиторий. Выберите один вариант запуска: Docker или локальное Python-окружение. Оба по умолчанию используют **http://localhost:3000**.
...
.venv/bin/python -m streamlit run app.py --server.address 127.0.0.1 --server.port 3000 -- --outputs outputs
```
...
Если validator сообщает о несовпадении SHA256, выполните пересчёт через `make run` / `.\make.cmd run`. Ранее на Windows обнаруживалась замена LF на CRLF в CSV; текущий [.gitattributes](.gitattributes) защищает опубликованные файлы от такого преобразования при checkout. Подробности — в [отчёте об ошибке SHA256](docs/audit-sha256-20260923.md).
...
6. Проверьте три конкретных случая из [семантического QA](docs/qa-real-data.md):
...
После добавления симуляции устойчивости: **182 passed за 92,80 с** на Windows / Python 3.12. Единственное предупреждение — об устаревании HTTPX-адаптера Starlette TestClient. Новый экран дополнительно проверен через Streamlit AppTest на официальном результате; расчёт top-5 занял около 0,04 с без загрузки и отрисовки. Docker и живой браузер в этом проходе заново не проверялись.
...
Аудит текущей публикации: 2 248 evidence и 20 why, несовпадений фактов не найдено; длина 64–110 символов. Скрипт только читает файлы. Смысловые ограничения и предложения улучшения — в [отчёте](docs/evidence-audit.md).

До текущего merge в ветке backend зафиксирован полный набор **111 passed** на Windows (70,65 с) и в Docker Linux (91,90 с), без четырёх тестов аудита evidence. Последующие изменения панели Streamlit дополнительно проверены шестью UI/graph-тестами в обеих средах. Ранее также выполнен сквозной HTTP-сценарий backend на официальных данных. Источники: [проверка backend](docs/backend-verification.md), [проверки UI](TECHNICAL_LANE_B.md), [аудит аналитики](AUDIT_LANE_A.md), [семантический QA](docs/qa-real-data.md), [проверки исходного CLI/Docker](docs/audit-sha256-20260923.md). Эти проверки подтверждают согласованность реализации, но не точность обнаружения нарушений.
...
### Эксперимент: что будет без ключевых узлов

В разделе **«Сеть и сообщества»** раскройте **«Устойчивость сети: исключение ключевых узлов»** и выберите 1, 3, 5 или 10 клиентов. Таблица сравнивает исходную сеть и два сценария: исключение по текущему приоритету и по наблюдаемому обороту (вход + выход). Показаны крупнейшая связная группа, число групп, новые изоляты, доля разобщённых пар оставшихся клиентов и суммы затронутых переводов. Списки исключённых gid доступны под таблицей.

Эксперимент использует полный bundle независимо от экранных фильтров. Связность слабая: направление игнорируется только для поиска компонент. Сумма направленного перевода считается один раз, даже если исключены оба его конца. Доля разобщённых пар учитывает только оставшихся клиентов, ранее связанных путём: простое уменьшение числа узлов не выдаётся за распад сети. Ранжирование фиксируется до исключения; превосходство приоритета над оборотом не предполагается заранее.

Это модель неполной наблюдаемой сети, не рекомендация блокировки счетов и не оценка предотвращённого ущерба. Адаптация участников и перенаправление потоков не моделируются. CSV, роли, приоритеты и исходные файлы не изменяются. Метод и ограничения: [методология](docs/methodology.md#устойчивость-сети-what-if).
...
- [Навигация по документации и состав очистки](docs/README.md)
- [Оценка качества evidence и воспроизводимый аудит](docs/evidence-audit.md)
- [Методология и план масштабирования до 1 млн узлов](docs/methodology.md)
- [Сценарий пятиминутного демо](docs/demo.md)
- [QA-чеклист](docs/qa.md)
- [Архитектура и протокол публикации](docs/architecture.md)
- [Технические команды UI и validator](TECHNICAL_LANE_B.md)
- [Актуальный чеклист сдачи](PLAN.md)
- [Контракт API и будущего frontend](docs/frontend-contract.md)
