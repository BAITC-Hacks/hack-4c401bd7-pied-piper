...
| `backend/core/resilience.py` | Read-only сравнение устойчивости полной сети при исключении ключевых узлов |
...
Историческая проверка улучшения evidence: **117 passed за 53,32 с** на Linux / Python 3.12; независимый аудит всех 2 248 клиентов прошёл. Пересчёт занял 1,90 с. В обновлённом Docker проверены объяснения всех шести ролей, поиск и скачивание трёх CSV; их SHA256 совпадают с локальной публикацией. Роли, оценки и графовые признаки при изменении текстов сохранены.
...
После объединения новой структуры, объяснений ролей и симуляции устойчивости: **186 passed за 72,07 с** на Windows / Python 3.12. Единственное предупреждение — об устаревании HTTPX-адаптера Starlette TestClient. Пересчёт официального набора во временный каталог занял 1,375 с; validator, аудит evidence и AppTest карточки/симуляции прошли. Локальный current.json сохранён без изменений. Docker и живой браузер в этом проходе заново не проверялись. Подробности — [QA](qa.md).
...
Аудит свежего проверочного расчёта: 2 248 evidence и 20 why, несовпадений фактов не найдено; длина 88–145 символов (лимит 200). Скрипт только читает файлы. Для обновления текстов в ранее сохранённой публикации нужен пересчёт через make pipeline / make run. [Исходный отчёт](evidence-audit.md) оценивает предыдущие шаблоны; он не является оценкой новых текстов.
...
Это модель неполной наблюдаемой сети, не рекомендация блокировки счетов и не оценка предотвращённого ущерба. Адаптация участников и перенаправление потоков не моделируются. CSV, роли, приоритеты и исходные файлы не изменяются. Метод и ограничения: [методология](methodology.md#устойчивость-сети-what-if).
